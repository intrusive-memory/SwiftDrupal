...
| Lifecycle Commands | `Sources/SwiftDrupal/CLI/Commands` (lifecycle) | 1 | 2 | Container Orchestration Core, Networking / Hostname Resolution |
| Database Import/Export | `Sources/SwiftDrupal/CLI/Commands` (database) | 1 | 2 | Container Orchestration Core |
| Dev Tools (exec/ssh/logs) | `Sources/SwiftDrupal/CLI/Commands` (dev tools), `Sources/SwiftDrupal/Logging` | 2 (6a, 6b) | 2 | Container Orchestration Core |
| Agent-Friendly Contract, Manifest & Docs | `Sources/SwiftDrupal` (cross-cutting), `AGENTS.md` | 2 (7a, 7b) | 3 | Lifecycle Commands, Database Import/Export, Dev Tools (exec/ssh/logs) |
...
**Entry criteria**:
- [ ] Sortie 2 exit criteria met (`ContainerService` and web/db spec builders available).
- [ ] Sortie 3 exit criteria met (`HostnameStrategy` and default selection available).
...
2. Implement `drupal start`: idempotent — pulls images if needed, creates/starts web+db containers via `ContainerService`, activates hostname resolution via the default `HostnameStrategy`, waits for health, returns JSON status.
...
**Entry criteria**:
- [ ] Sortie 2 exit criteria met (`ContainerService` available to exec into the db container).
...
**Entry criteria**:
- [ ] Sortie 2 exit criteria met (`ContainerService` available for exec access).
...
**Entry criteria**:
- [ ] Sortie 6a exit criteria met (dev-tools work unit sequencing; log streaming reuses the same `ContainerService` exec/log access wired up there).
...
**Critical Path**: Sortie 1 → Sortie 2 → Sortie 6a → Sortie 6b → Sortie 7a → Sortie 7b (length: 6 sorties)
...
- **Group 3** (Layer 2 — can run in parallel, all depend on Sortie 2; Lifecycle also depends on Sortie 3):
...
- **Group 4** (Layer 3 — sequential, depends on all of Group 3):
...
| Work units | 7 |
| Total sorties | 9 |
| Open questions | 0 (2 resolved in Decision Log) |
